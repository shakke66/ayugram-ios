...
private func removeChat(transaction: Transaction, postbox: Postbox, network: Network, stateManager: AccountStateManager, peer: Peer, operation: CloudChatRemoveChatOperation) -> Signal<Void, NoError> {
    if peer.id.namespace == Namespaces.Peer.CloudChannel {
        ...
        |> then(reportSignal)
        |> then(postbox.transaction { transaction -> Void in
            _internal_clearHistory(accountPeerId: stateManager.accountPeerId, transaction: transaction, mediaBox: postbox.mediaBox, peerId: peer.id, threadId: nil, namespaces: .all)
        })
        ...
        if let inputPeer = apiInputPeer(peer) {
            ...
            |> then(reportSignal)
            |> then(postbox.transaction { transaction -> Void in
                _internal_clearHistory(accountPeerId: stateManager.accountPeerId, transaction: transaction, mediaBox: postbox.mediaBox, peerId: peer.id, threadId: nil, namespaces: .not(Namespaces.Message.allNonRegular))
            })
        } else {
            return .complete()
        }
