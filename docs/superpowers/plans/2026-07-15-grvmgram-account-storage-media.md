...
Declare an explicit `public init` for every public record above. Swift's synthesized memberwise initializer is `internal`, while `GRVMMessageArchiveCoordinator` lives in `AyuGramFeatures` and must construct these AyuGramLib values.
